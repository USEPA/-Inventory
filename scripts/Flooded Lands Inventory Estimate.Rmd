...
# from US_Reservoirs3.gdb provided by Hall on 8/24/2021
inund <- sf::st_read(paste0("C:\\Users\\JBEAULIE\\Environmental Protection Agency (EPA)\\",
                            ...
                            "surgeDsn\\nationalFloodedLandsMapping\\Reservoir Data\\US_Reservoirs3.gdb"),
                     layer = "Inundation_Areas") %>%
  st_transform(4269)

# from Data.gdb provided by Alex on 1/10/2020
nav <- sf::st_read(paste0("C:\\Users\\JBEAULIE\\Environmental Protection Agency (EPA)\\",
  ...
  filter(FTYPE != "SeaOcean")
st_crs(nav) #4269

# from US_Reservoirs2.gdb provided by Hall on 8/12/2021
...
nrow(inund) + nrow(nav) + nrow(res) #54,708

# combine spatial data object
fl.sf <- dplyr::bind_rows(nav, inund, res) 
nrow(fl.sf) #54708, all observations retained
st_crs(fl.sf) #4269
```
...
fl.sf %>% filter(COMID == 13335116) %>% # grab Mississippi Pool 8 
  select(area_ha_original, Area_ha) # great, looks good!

# remove other area related variables for simplicity
area_names <- names(fl.sf)[grepl(pattern = "area", x = names(fl.sf), ignore.case = TRUE)] # all names with "area"
area_names <- area_names[!(area_names == "area_ha_original")] # want to keep 'area_ha_original'
fl.sf <- fl.sf %>% select(-area_names)
...
us <- us_states() %>% # get states map
  # filter(name %in% c("Hawaii", "Alaska") %>% # I think we use all states and DC
  st_transform(crs = 4269) # 4269, as above.  Not certain what warning is about.
...
climate <- sf::st_read("inputData/climateMap", layer = "ipcc_zones_2017_names") %>%
  ...
  rename(climate = fldd_zn) %>% # rename
   st_transform(4269) # consistent with above
...
fl.sf.c %>% filter(COMID == 13335116) %>% # grab Mississippi Pool 8 
  select(name, area_ha_original, area_ha_new) # great, looks good!
...
lapply(fl, function(x) nrow(x)) #55284 rows
...
lapply(fl, function(x) nrow(x)) #55,284 in 2020, but only 54,824 in 1990. good
unique(fl[[29]]$lu)
table(fl[[29]]$lu)
```
...
us.plot.flrfl.total.ch4.2020.reservoir <- st_transform(us.plot.flrfl.total.ch4.2020.reservoir, 5070)


ggplot(us.plot.flrfl.total.ch4.2020.reservoir) +
  geom_sf(aes(fill = ch4.total.ktonnes.y)) +
  scale_fill_distiller(expression(CH[4]~flux~(kt~y^-1)), 
