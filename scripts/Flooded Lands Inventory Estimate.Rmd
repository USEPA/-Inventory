...
fl.cl.area <- lapply(fl.e, function(x) {
  ...
  x %>% 
    st_drop_geometry() %>%
    dplyr::group_by(lu, subtype, name, climate) %>% # 
...
### DEPRECATED------------------------
### Code for writing outnationally aggregated data to Excel.  This output is
...
# MUST CONVERT TO KT.  DIVIDE BY 1000!
fl.e$`2020` %>%  
  st_drop_geometry() %>%
...
In 2019 the conterminous United States hosted `r fl.cl.area.state.type %>% filter(lu == "Flooded Land Remaining Flooded Land", type == "other constructed waterbodies") %>% group_by(subtype) %>% summarize(tot.area = sum(area2019))`  hectares of freshwater pond and `canals and ditches` surface area, respectively, in the Flooded Land Remaining Flooded Land category.    
...
p1 <- ggplot(us.plot.flrfl.total.ch4.2020.ocwb) +
  ...
  scale_fill_distiller(expression(kt~CH[4]~y^-1), 
                       palette = "Blues", direction = -1) +
  coord_sf(datum = NA) + # this removes grid lines.
  ggtitle(expression(A.~CH[4]~Flux~from~Canals~and~Ditches)) +
  theme(panel.background = element_blank(),
...
p2 <- ggplot(us.plot.flrfl.total.ch4.2020.ocwb) +
  ...
  scale_fill_distiller(expression(kt~CH[4]~y^-1), 
                       palette = "Blues", direction = -1) +
  coord_sf(datum = NA) + # this removes grid lines.
  ggtitle(expression(B.~CH[4]~Flux~from~Freshwater~Ponds)) +
  theme(panel.background = element_blank(),
        title = element_text(size = 8))
...
ggsave("output/figures/FLRFL.ocwb.2020.Tch4.tiff", g)
```
...
summary(sim0.085)
plot(density(sim1.4)) # this seems pretty good
...
ef.error <- readxl::read_excel("inputData/defaultEmissionFactors.xlsx") %>%
  ...
  # This will allow paramter distribution to be defined using rnorm, the preferred IPCC method.
...
# record nrow of fl list prior to merge.  Should be identical after merge.
n.row.prior <- nrow(fl$`2020`) #55597

# Merge emission factors with each list element.  This takes a few minutes
fl.error <- merge(fl$`2020`, ef.error)  
...
for (i in 1:200) {
  poo <- fl.error %>% 
    mutate(co2.tonnes.ha.y = runif(nrow(fl.error), co2.tonnes.ha.y.lwr, co2.tonnes.ha.y.upr),
           ...
           # confidence interval less than mean estimate
...
ch4.co2.mean.table <- fl.e$`2020` %>%  
  ...
  mutate(subtype = ifelse(subtype == "inundation area", # not discriminating inundation and reservoir here
                          "reservoir",
                          subtype)) %>%
  select(lu, subtype, ch4.surface.tonnes.y, ch4.dn.tonnes.y, ch4.total.tonnes.y, co2.tonnes.y) %>%
  group_by(lu, subtype) %>%
...
bound.perc = function(x,y){((x - y)/y) *100}
...
ch4.co2.uncertainty.mean.table %>% filter(lu == "Flooded Land Remaining Flooded Land",
                                          subtype == "reservoir")
