...
tiff("output/figures/newDams.tiff", res = 800, compression = "lzw", width = 4.77/2, height = 4.99/2, units = "in")
...
for (i in 1:1000) { # 1000 simulations takes 50 seconds
  poo <- fl.error %>% 
...
ch4.co2.uncertainty.mean.table %>% filter(lu == "Land Converted to Flooded Land",
                                          subtype == "reservoir") %>% 
  write.table(., "clipboard", row.names = FALSE)
```
